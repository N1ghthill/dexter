...
describe('temporal-intelligence', () => {
  ...
  it('responde data atual com referencia situacional', () => {
    const reply = tryBuildTemporalDeterministicReply({
    ...
    expect(reply).toContain('Hoje e');
    expect(reply).toContain('24 de fevereiro de 2026');
    expect(reply).toContain('America/Sao_Paulo');
  });

  it('usa fallback seguro quando timezone informado e invalido', () => {
    const reply = tryBuildTemporalDeterministicReply({
      input: 'Quantos dias restam para acabar esse ano?',
      now: new Date('2026-12-30T08:00:00.000Z'),
      locale: 'pt-BR',
      timeZone: 'Invalid/Zone'
    });

    expect(reply).toContain('(Invalid/Zone)');
    expect(reply).toContain('faltam 1 dia');
    expect(reply).toContain('31 de dezembro de 2026');
  });
