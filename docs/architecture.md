...
Para reduzir acoplamento no `renderer`, o entrypoint `src/renderer/main.ts` monta o shell React e delega a orquestracao legada de eventos/IPC para `src/renderer/legacy-main.ts`. A ponte entre componentes React e funcoes legadas e feita por intents (`src/renderer/state/ui-intents.ts`), permitindo migracao incremental sem quebra de contrato.
